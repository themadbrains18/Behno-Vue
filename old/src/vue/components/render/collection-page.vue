      ...
      <div class="product_item" v-for="(value, key) in Products" :key="key">
        ...
        <div class="card" v-if="(value.hasOwnProperty('single'))">
          <a :href="(`/products/`+value.single.handle)">
            ...
            <div class="item_left item_left_active {{ key }}">
            </div>
            <div
              class="product_img_wrapper"
              id="product_img_wrapper6617090588769"
            >
               <img :src=(value.single.images.edges[0].node.transformedSrc) alt="{{ value.single.images.edges[0].node.altText }}" class="normal" />
             <img :src=(value.single.images.edges[1].node.transformedSrc) alt="{{ value.single.images.edges[0].node.altText }}"  class="hoverImg" />
          ...
          <div class="quickButton quickActive">
            ...
            <div class="product_cta_wrapper" data-v-32bfb114="">
              <button
                id="quickAdd6617090588769"
                class="quickAdd"
                data-v-32bfb114=""
              >
                <span data-v-32bfb114="">Quick Add</span>
              </button>
            </div>
          </div>
        </div>
        ...
        <div class="card" v-if="(value.hasOwnProperty('variable'))">
          <a :href="(`/products/`+value.variable[0].handle)">
            ...
            <div class="item_left item_left_active">
            </div>
            <div
              class="product_img_wrapper"
              id="product_img_wrapper6617090588769"
            >
               <img :src=(value.variable[0].images.edges[0].node.transformedSrc) alt="{{ value.variable[0].images.edges[0].node.altText }}" class="normal" />
              <img :src=(value.variable[0].images.edges[1].node.transformedSrc) alt="{{ value.variable[0].images.edges[0].node.altText }}"  class="hoverImg" />
          ...
          <div class="quickButton quickActive">
            <div class="color_swatches">
                 <ul :mainKey="(JSON.stringify(value))">
                     ...
                     <li v-for="(sValue, sKey) in value.swatches" :key="sKey" :index="sKey" :link="(sValue.link)" @click="selectVariation" class="nav-dots" v-bind:class = "(sKey == 0)?'active':''">
                           <span>
...
import roughdata from "../../../assets/graphql/roughdata.json";
...
export default {
  ...
  methods: {
    async fetchProdustQuery() {
      ...
      //   data: JSON.stringify(queries.queryBuilder()),
      // };
      ...
      //     localStorage.setItem("productLists", response)
      // } else {

      // }

      // save if use load first time

      let response = {};

      response.data = roughdata;
      ...
      // if(response.data.data.collectionByHandle.products.pageInfo.hasNextPage == ture){
      //     this.shopifyPagination = true  // active to paginatio button
      // }

      var products = response.data.data.collectionByHandle.products.edges;
      ...
      for (let product in products) {
        ...
        var links = products[product].node.metafields.edges[0].node.value; /// product pair links
        ...
        if (bindInPair.length > 0) {
          ...
          collectPairProducts.push(products[product].node);

          for (let childPair in products) {
            if (bindInPair.includes(products[childPair].node.handle)) {
      ...
      this.Products = [...productListing].slice(0, 100);
    },
    ...
    selectVariation: (event) => {
       ...
       // update product data 
         ...
         grid.getElementsByTagName('a')[0].setAttribute('href',activeProduct.handle)  // set anchore tag
         grid.querySelector('img.normal').setAttribute('src',activeProduct.featuredImage.transformedSrc)
         grid.querySelector('img.hoverImg').setAttribute('src',activeProduct.images.edges[1].node.transformedSrc)
    ...
    getThemeAssets(image) {
      ...
      for (let images in assetsLIst) {
        ...
        if (assetsLIst[images].key == "assets/" + image) {
          console.log(assetsLIst[images].public_url)
