<template>
  ...
  <section class="sec_partnership_with_ndss">
    <div class="big_container">
      ...
      <div class="sec_video">
        <template v-if="shopifyData.checkImage === 'true'">
          ...
          <template v-else>
            <v-lazy-image  :src=(shopifyData.imgUrl.src) :src-placeholder=(shopifyData.imgUrl.placeholder) :alt=(shopifyData.imgUrl.alt) />
          </template>
      </div>
    </div>
  </section>
...
.sec_partnership_with_ndss .big_container {
  padding: 0 33px;
}
.quotes_text {
  color: #ef0000;
