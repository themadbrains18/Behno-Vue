  ...
  <section class="product_sec">
    <div class="product_grid">
      ...
      <div class="product_grid_content">
        <h1 class="product_title card_heading">
        ...
        >
          <p class="after_pay">
            or 4 interest-free installments of $97.50 by<img src="https://cdn.shopify.com/s/files/1/0577/1178/8125/files/afterpay.png?v=1653477636">
          </p>
          <ul class="product_variant">
            <li
              v-for="(value, key) in variant"
              :key="key"
              class="color_variant_wrap"
              @click="changePath(value.link)"
            >
          ...
          <div
            v-if="selectedProduct.variants.length > 1"
            class="select"
          >
            <label id="selectSize">Select Size</label>
            <ul class="swatchs_warraper">
              <li
                v-for="(value, key) in selectedProduct.variants"
                :key="key"
                @mouseenter="sizeSelect(value.title,'hover')"
                @mouseleave="sizeSelect('Select Size','leave')"
                @click="sizeSelect(value.title,'click')"
              >
                {{ value.title }}
              </li>
            </ul>
          ...
          <p
            ...
            class="product_left subtitle_b"
          >
            Out of stock
          </p>
          <p
            v-else-if="currentVariantQty <= 5"
            class="product_left subtitle_b"
          >
            ONLY {{ currentVariantQty }} LEFT
          </p>
          <input
            name="id"
        ...
        <div class="product_description_data">
          ...
          <ul class="product_accordians">
            <!-- Product Details -->
            <li
              v-if="selectedProductDescription[1]"
              class="product_accordian"
            >
              <button
                class="accodian d-flex w-100"
                @click="(event)=>{accordion(event)}"
              >
                <svg
                  width="40"
                  height="40"
                ...
                >
                  <path
                    d="M28.7969 16.1564C29.2245 15.7257 29.5537 15.2073 29.7616 14.6371C29.9696 14.0669 30.0514 13.4583 30.0015 12.8534C29.9517 12.2485 29.7712 11.6616 29.4726 11.1332C29.1741 10.6048 28.7645 10.1473 28.2721 9.79245C27.7797 9.43756 27.2162 9.19366 26.6204 9.07755C26.0247 8.96143 25.4108 8.97586 24.8212 9.11985C24.2316 9.26384 23.6802 9.53396 23.205 9.91159C22.7298 10.2892 22.3422 10.7654 22.0688 11.3073C22.0211 11.4032 22.0135 11.5141 22.0476 11.6157C22.0817 11.7172 22.1547 11.801 22.2506 11.8488C22.346 11.8967 22.4566 11.9049 22.558 11.8716C22.6595 11.8383 22.7436 11.7661 22.7921 11.671C22.9567 11.344 23.1722 11.0453 23.4306 10.786C23.8274 10.388 24.317 10.0948 24.8552 9.933C25.3935 9.77116 25.9635 9.74567 26.5141 9.85884C27.0646 9.972 27.5784 10.2203 28.0092 10.5813C28.44 10.9423 28.7742 11.4048 28.9819 11.9271C29.1896 12.4494 29.2642 13.0151 29.1989 13.5733C29.1337 14.1316 28.9306 14.6649 28.6081 15.1252C28.2856 15.5855 27.8537 15.9584 27.3513 16.2104C26.8488 16.4624 26.2917 16.5854 25.7299 16.5686L25.5197 13.8369C25.5131 13.7394 25.4714 13.6476 25.4023 13.5785C25.3332 13.5094 25.2414 13.4677 25.1439 13.4611L18.2016 12.9964C18.1437 12.9922 18.0856 13.0004 18.0312 13.0206C17.9768 13.0409 17.9274 13.0726 17.8864 13.1136L9.11751 21.8865C9.04224 21.9622 9 22.0646 9 22.1714C9 22.2781 9.04224 22.3806 9.11751 22.4563L16.5407 29.8795C16.5782 29.9177 16.6228 29.948 16.672 29.9689C16.7213 29.9897 16.7742 30.0005 16.8276 30.0007C16.9342 29.9992 17.0359 29.9556 17.1105 29.8795L25.8834 21.1106C25.9244 21.0696 25.9561 21.0202 25.9763 20.9658C25.9966 20.9114 26.0048 20.8533 26.0006 20.7954L25.7662 17.3849H25.8107C26.3651 17.3889 26.9148 17.2823 27.4275 17.0714C27.9403 16.8605 28.4058 16.5494 28.7969 16.1564ZM25.2005 20.6702L16.8276 29.0228L9.95803 22.1532L18.3268 13.8369L24.756 14.2612L24.9095 16.4474C24.3503 16.2935 23.8406 15.9969 23.4306 15.5866C23.3532 15.5204 23.2538 15.4859 23.1521 15.4898C23.0504 15.4937 22.9539 15.5359 22.882 15.6078C22.81 15.6798 22.7678 15.7763 22.7639 15.878C22.76 15.9797 22.7946 16.0791 22.8608 16.1564C23.4365 16.7333 24.1683 17.1294 24.9661 17.296L25.2005 20.6702Z"
                    fill="black"
                  />
                </svg>
                DETAILS
              ...
              <div
                ...
                v-html="replaceString(selectedProductDescription[1])"
              />
              <!-- eslint-enable -->
            </li>
            <!-- Product Specification -->
            <li
              v-if="selectedProductDescription[2]"
              class="product_accordian"
            >
              <button
                class="accodian d-flex w-100"
              ...
              <div
                class="product_accordian_panel"
                v-html="replaceString(selectedProductDescription[2])"
              />
              <!-- eslint-enable -->
            </li>
            <!-- Product Shipping -->
            <li
              v-if="selectedProductDescription[3]"
              class="product_accordian"
            >
              <button
                class="accodian d-flex w-100"
              ...
              <div
                class="product_accordian_panel"
                v-html="replaceString(selectedProductDescription[3])"
              />
              <!-- eslint-enable -->
            </li>
            <!-- Product ETHICAL PHILOSOPHY  -->
            <li
              v-if="selectedProductDescription[4]"
              class="product_accordian"
            >
              <button
                class="accodian d-flex w-100"
      ...
      <div
        id="productZoom"
        class="product_zoom modify-slider line-h-0"
      >
        <swiper
          :modules="modules"
...
export default {
    ...
    data() {
        ...
        return {
            ...
            currentVariantQty : parseInt(filterVariant.qty)
        }
    },
    mounted() {
        ...
        sizeSelect(size,type){
            ...
            if(type == 'leave'){
                if(this.selectedSize!=""){
                    label.innerHTML = this.selectedSize;
                }
            }
        },
        changePath(link){
