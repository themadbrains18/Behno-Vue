                ...
                <form @submit="onAddtoCart"  method="post" id="productForm">
                    ...
                    <div class="select" v-if="this.selectedProduct.variants.length > 1">
                        ...
                        <ul class="swatchs_warraper">
                            <li v-for="(value, key) in this.selectedProduct.variants" :key="key" v-on:mouseenter="sizeSelect(value.title,'hover')" v-on:mouseleave="sizeSelect('Select Size','leave')" v-on:click="sizeSelect(value.title,'click')">
                                {{value.title}}
                            </li>
                        </ul>
                    </div>
